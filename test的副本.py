...
xgtrain = xgb.DMatrix(train_df_munged, label=label_df)
xgtest = xgb.DMatrix(test_df_munged)

#num_rounds = 10000
#res = xgb.cv(params, xgtrain, num_boost_round=num_rounds, nfold=20, stratified=False,
#             early_stopping_rounds=100, verbose_eval=100, show_stdv=False)
...
best_nrounds = 3428
regr = xgb.train(params, xgtrain, best_nrounds)

# Run prediction on the Kaggle test set.
xgb_preds = regr.predict(xgtest, ntree_limit=regr.best_ntree_limit)
################################################################################

from sklearn.linear_model import LassoCV, ElasticNet

#LASSO MODEL
regr = LassoCV(alphas = [1, 0.1, 0.001, 0.0005, 5e-4], max_iter=50000)
regr.fit(train_df_munged, label_df["SalePrice"].values)
lasso_preds = regr.predict(test_df_munged)

#ELASTIC NET
regr = ElasticNet(alpha=0.0005, l1_ratio=0.9)
regr.fit(train_df_munged, label_df["SalePrice"].values)
elas_preds = regr.predict(test_df_munged)

################################################################################

# Blend the results of the two regressors and save the prediction to a CSV file.

y_pred = (xgb_preds + lasso_preds + elas_preds) / 3
y_pred = np.exp(y_pred)

pred_df = pd.DataFrame(y_pred, index=test_df["Id"], columns=["SalePrice"])
pred_df.to_csv('sub_blend.csv', header=True, index_label='Id')
